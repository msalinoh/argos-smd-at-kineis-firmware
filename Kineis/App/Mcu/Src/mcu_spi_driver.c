...
static SPI_Buffer rxBuf = { .data = spiRxBuf, .size = RXBUF_SIZE, .next_req = 1};
static SPI_Buffer txBuf = { .data = spiTxBuf, .size = TXBUF_SIZE, .next_req = 0};


// SPI Status Byte Definition
uint8_t spi_status = (0 | SPI_OK);
...
//        		if (txBuf.next_req > 1)
//        		{
//					spi_status = SPI_OK | SPI_TX_WAITING;
//	//        		HAL_SPI_Transmit_IT(hspi, txBuf.data, txBuf.next_req);
//					HAL_SPI_TransmitReceive_IT(hspi, txBuf.data, rxBuf.data, txBuf.next_req);
//        		} else {
//					spi_status = SPI_OK | SPI_TX_WAITING;
//					HAL_SPI_Transmit_IT(hspi, txBuf.data, txBuf.next_req);
//        		}
//        	} else {
//        		//Re start receive command if an error occured
//				rxBuf.next_req = 1;
...
//	{
		...
		if(rxBuf.next_req > 0)
		{
			spi_status = SPI_OK | SPI_RX_WAITING;
			ret = HAL_SPI_Receive_IT(hspi_handle, rxBuf.data, rxBuf.next_req);
		} else {
...
//	}
//	else {
//		MGR_LOG_DEBUG("%s:: cmd  is %u should be different than 0\n\r",__func__, rxBuf.next_req);
//		ret = HAL_ERROR;
//	}
	return ret;
}
...
bool MCU_SPI_DRIVER_register(void *handle, int8_t (*rx_spi_evt_cb)(SPI_Buffer *rx, SPI_Buffer *tx))
{
	...
	ret = HAL_SPI_Receive_IT(hspi_handle, rxBuf.data, rxBuf.next_req);
	//ret = HAL_SPI_TransmitReceive_IT(hspi_handle, txBuf.data, rxBuf.data, rxBuf.next_req);
